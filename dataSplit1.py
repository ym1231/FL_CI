...
def split_image_data(data, labels, n_clients=100, classes_per_client=10, shuffle=True, verbose=True):
    ...
    #class_split = print_split(clients_split)

    clients_split = np.array(clients_split)
    

    return clients_split, class_split
...
def get_data_loaders(nclients, batch_size, real_wd, ratio, weighted=True, dataset='cifar'):
    ...
    class_split = np.zeros((nclients, 10))
    
    
    #def cal_split(clients_split)
    for i, client in enumerate(split):
            split1 = np.sum(client[1].reshape(1, -1) == np.arange(10).reshape(-1, 1), axis=1)
            class_split[i,:] = split1
    ...
    return client_loaders, test_loader, data_size_per_client, class_split
